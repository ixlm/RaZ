...
public:
  ...
  float computeLength() const { return (m_endPos - m_beginPos).Length(); }
  /// Line squared length computation.
  /// To be preferred over computeLength() for faster operations.
  /// \return Line's squared length.
  float computeSquaredLength() const { return (m_endPos - m_beginPos).LengthSquared(); }
...
public:
  ...
  explicit Plane(float distance, const Vec3f& normal = {0.f, 1.f, 0.f}) : m_distance{ distance }, m_normal{ normal } {}
  // explicit Plane(const Vec3f& position, const Vec3f& normal = Axis::Y) : m_distance{ position.computeLength() }, m_normal{ normal } {}
  explicit Plane(const Vec3f& position, const Vec3f& normal = {0.f, 1.f, 0.f}) : m_distance{ position.Length() }, m_normal{ normal } {}
  ...
  Plane(const Vec3f& firstPoint, const Vec3f& secondPoint, const Vec3f& thirdPoint)
    : m_distance{ ((firstPoint + secondPoint + thirdPoint) / 3.f).Length() },
      m_normal{ CrossProductHelper((secondPoint - firstPoint), (thirdPoint - firstPoint)).Normalize() } {}
  ...
  { 
    // return FloatUtils::areNearlyEqual(m_normal.Dot(point) - m_distance, 0.f); 
    return FloatUtils::areNearlyEqual(DotProductHelper(m_normal, point)  - m_distance, 0.f); 
  }
  ...
  Vec3f computeProjection(const Vec3f& point) const override { return point - m_normal * (DotProductHelper( m_normal, point) - m_distance); }
...
public:
  ...
  bool isCounterClockwise(const Vec3f& normal) const { return ( DotProductHelper(normal, computeNormal()) > 0.f); }
